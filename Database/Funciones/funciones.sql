...
CREATE FUNCTION fn_calcular_monto_ejecutado(p_id_subcategoria VARCHAR(30), p_anio INT, p_mes INT)
...
CREATE FUNCTION fn_calcular_porcentaje_ejecutado(p_id_subcategoria VARCHAR(30), p_id_presupuesto VARCHAR(30), p_anio INT, p_mes INT)
RETURNS DECIMAL(12,2)
DETERMINISTIC
BEGIN
...
CREATE FUNCTION fn_obtener_balance_subcategoria(p_id_presupuesto VARCHAR(30), p_id_subcategoria VARCHAR(30), p_anio INT, p_mes INT)
RETURNS DECIMAL(12,2)
DETERMINISTIC
BEGIN
  DECLARE v_ejecutado DECIMAL(12,2);
...
CREATE FUNCTION fn_obtener_total_categoria_mes(p_id_categoria VARCHAR(30), p_id_presupuesto VARCHAR(30), p_anio INT, p_mes INT)
RETURNS DECIMAL(12,2)
DETERMINISTIC
BEGIN
  DECLARE v_total DECIMAL(12,2);
...
CREATE FUNCTION fn_obtener_total_ejecutado_categoria_mes(p_id_categoria VARCHAR(30), p_anio INT, p_mes INT)
RETURNS DECIMAL(12,2)
DETERMINISTIC
BEGIN
  DECLARE v_total DECIMAL(12,2);
...
CREATE FUNCTION fn_dias_hasta_vencimiento(p_id_obligacion VARCHAR(30))
RETURNS INT
DETERMINISTIC
BEGIN
  DECLARE v_dia INT;
  ...
  SET v_target = STR_TO_DATE(CONCAT(YEAR(v_hoy), '-', LPAD(MONTH(v_hoy),2,'0'), '-', LPAD(v_dia,2,'0')), '%Y-%m-%d');

  RETURN DATEDIFF(v_target, v_hoy);
END//

DROP FUNCTION IF EXISTS fn_validar_vigencia_presupuesto//
CREATE FUNCTION fn_validar_vigencia_presupuesto(p_fecha DATE, p_id_presupuesto VARCHAR(30))
RETURNS TINYINT
DETERMINISTIC
BEGIN
  DECLARE v_ini DATE;
...
CREATE FUNCTION fn_obtener_categoria_por_subcategoria(p_id_subcategoria VARCHAR(30))
RETURNS VARCHAR(30)
DETERMINISTIC
BEGIN
  DECLARE v_cat VARCHAR(30);

  SELECT id_categoria INTO v_cat
  FROM subcategoria
  WHERE id_subcategoria = p_id_subcategoria;
...
CREATE FUNCTION fn_calcular_proyeccion_gasto_mensual(p_id_subcategoria VARCHAR(30), p_anio INT, p_mes INT)
RETURNS DECIMAL(12,2)
DETERMINISTIC
BEGIN
  DECLARE v_ejecutado DECIMAL(12,2);
  ...
  SET v_dias_mes = DAY(LAST_DAY(STR_TO_DATE(CONCAT(p_anio,'-',LPAD(p_mes,2,'0'),'-01'), '%Y-%m-%d')));
...
CREATE FUNCTION fn_obtener_promedio_gasto_subcategoria(p_id_usuario VARCHAR(30), p_id_subcategoria VARCHAR(30), p_cantidad_meses INT)
RETURNS DECIMAL(12,2)
DETERMINISTIC
BEGIN
